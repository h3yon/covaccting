...
async function selectReviewCreate(
  ...
  try {
    ...
    select Result.reviewIdx, Result.userIdx, userNickname, userProfileImgLink,
       Result.reviewContent, Result.vaccineName, likeReviewCount, likeStatus, Result.createdAt
    from (select Review.reviewIdx as reviewIdx,
            Review.userIdx as userIdx,
            User.userNickname,
            User.userProfileImgLink,
            reviewContent,
            vaccineName,
            (select count(*)
            from LikeReview
            where LikeReview.status = 1 and LikeReview.reviewIdx = Review.reviewIdx) as likeReviewCount,
            (select CASE WHEN LikeReview.status = 1 THEN 1 ELSE 0 END)                as likeStatus,
            case
                when TIMESTAMPDIFF(MINUTE, Review.createdAt, now()) < 1
                    then concat(TIMESTAMPDIFF(SECOND, Review.createdAt, now()), '초전')
                when TIMESTAMPDIFF(HOUR, Review.createdAt, now()) < 1
                    then concat(TIMESTAMPDIFF(MINUTE, Review.createdAt, now()), '분전')
                when TIMESTAMPDIFF(DAY, Review.createdAt, now()) < 1
                    then concat(TIMESTAMPDIFF(HOUR, Review.createdAt, now()), '시간전')
                when TIMESTAMPDIFF(WEEK, Review.createdAt, now()) < 1
                    then concat(TIMESTAMPDIFF(DAY, Review.createdAt, now()), '일전')
                when TIMESTAMPDIFF(MONTH, Review.createdAt, now()) < 1
                    then concat(TIMESTAMPDIFF(WEEK, Review.createdAt, now()), '주전')
                when TIMESTAMPDIFF(YEAR, Review.createdAt, now()) < 1
                    then concat(TIMESTAMPDIFF(MONTH, Review.createdAt, now()), '달전')
    ...
    where if(? is not null, vaccineName = ?, 1 = 1)
      and Review.status = 1
      and User.status = 1
    union
    select Review.reviewIdx as reviewIdx,
            Review.userIdx as userIdx,
            User.userNickname,
            User.userProfileImgLink,
            reviewContent,
    ...
    left join Review on Result.reviewIdx = Review.reviewIdx
